...
import { validateCreateOrUpdateUrl } from "../middleware/createOrUpdate.url";
...
router.post("/url",validateCreateOrUpdateUrl,authenticateOptional(false), (req,res) => {
    urlController.create(req,res);
});

router.get("/urls", authenticateOptional(true),(req,res) => urlController.listMyUrls(req,res));
router.put("/urls/:id", validateCreateOrUpdateUrl,authenticateOptional(true),(req,res) => urlController.updateMyUrl(req,res));
